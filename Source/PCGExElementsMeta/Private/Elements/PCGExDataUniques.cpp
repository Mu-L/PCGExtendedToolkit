...
namespace PCGExDataUniques
{
	...
	// stay distinct. False when a key is missing or has a container/extended type -- ComputeHash returns 0 for
	// those, which would silently collapse every such input into one bucket. Read-only, safe to run in parallel.
	bool HashKeys(const FPCGExDataUniquesContext* Context, const UPCGData* InData, PCGExValueHash& OutHash, bool& bOutUnsupported)
	{
		...
		for (const FPCGAttributeIdentifier& Identifier : Context->KeyIdentifiers)
		{
			...
			PCGExMetaHelpers::ExecuteWithRightType(
				...
				[&](auto DummyValue)
				{
					...
					ValueHash = PCGExTypeOps::ComputeHash<T>(PCGExData::Helpers::ReadDataValue<T>(Attribute));
					bHashed = true;
				},
				[&]()
				{
